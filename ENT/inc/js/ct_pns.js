let id = 1;
const labelR = [5, 12, 18, 21, 26, 32, 36, 39, 44, 50, 56, 61, 64, 66, 69, 72, 76, 79, 84, 89, 94, 97, 103, 106, 113, 117, 119]; //array of labels (total 27)
let labelON = false;
...
//     targetImg.src = "assets/pns_coronal/img" + id + ".jpg";
// }

function showLabel(obj) {
  // var id = obj.children[0].src.match(/.*img(\d+)\.jpg/)[1];
  // labelImg.src = "assets/pns_coronal/labels/" + getLabelID(id) + ".png";
  if (labelON) {
...
function changeImageById(val) {
    targetImg.src = targetImgJPG.srcset = "assets/pns_coronal/img" + val + ".jpg";
    targetImgWebp.srcset = `assets/pns_coronal/img${val}.webp`;

    labelImg.src = labelImgPNG.srcset = `assets/pns_coronal/labels/${getLabelID(val)}.png`;
    labelImgWebp.srcset = `assets/pns_coronal/labels/${getLabelID(val)}.webp`;
    line.style.left = (15 + (val*3.144)).toString() + "px"; // offset from left (in css) + (width of image - offset)/number of    
} 
...
function changeSagitalOnClick(elem,event) {
  ...
  if (elem.srcElement.id === "sagplus" && sag_id < 58)
    sag_id++;
...
function changeSagital(sag_id) {
  sagImg.src = sagImgJPG.srcset = "assets/pns_coronal/ref/img" + sag_id + ".jpg"
  sagImgWebP.srcset = "assets/pns_coronal/ref/img" + sag_id + ".webp"
}
